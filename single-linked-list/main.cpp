...
public:
    ...
    SingleLinkedList& operator=(const SingleLinkedList& rhs) {
        if(this != &rhs){
            SingleLinkedList placeholder(rhs);
            swap(placeholder);
        }
        return *this;
    }
...
template <typename Type>
bool operator<(const SingleLinkedList<Type>& lhs, const SingleLinkedList<Type>& rhs) {
    auto i_rhs = rhs.begin();
    for(auto i_lhs = lhs.begin(); i_lhs != lhs.end(); ++i_lhs){
        if(*i_lhs < *i_rhs){
            return true;
        }
        else if(*i_lhs > *i_rhs || i_rhs == rhs.end()){
            return false;
        }
        ++i_rhs;
    }
